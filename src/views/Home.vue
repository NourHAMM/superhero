...
<input :value= "searchItem"  type="text" class="form-input mt-1 block w-full" placeholder="Search a Superhero, example: Thor" 
@change="setSearchItem"
/>
</label>
<div class="flex flex-col flex-wrap justify-center items-center">
<PulseLoader v-if="loading" class="mt-2" color="#ECC94B" size="2em" />
   <h1 v-if="searchItem" class="w-full text-center px-4 md:px-16 text-teal-200 font-semibold text-3xl md:text-4xl lg:text-5xl xl:text-6xl">
      ...
      <Card class="sm:w-1/2 lg: *l:w-1/4 p-2" v-for="hero in heroes" :key="hero.id" :hero="hero"  />
      </div>
    </div>
  </div>
</template>
